...
export default (env, argv) => {
    ...
    const proxy = "http://www.private.loc/altrd8-wp-starter/";

    // Define the entry points for the webpack build
    const entry = {
        bundle: "./static/js/index.js",
        style: ["./static/scss/style.scss"],
        // CSS-only entry for the block editor (wp-admin). No JS import here —
        // RemoveEmptyScriptsPlugin drops the otherwise-empty editor.js output.
        // Enqueued only via enqueue_block_editor_assets, never on the front end
        // (see App\bundles\Altrd8WpStarterAssets::register_editor()).
        editor: "./static/scss/editor.scss",
    };
